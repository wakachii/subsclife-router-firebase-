...
import { doc, getDocs,collection, addDoc,setDoc,onSnapshot,updateDoc} from "firebase/firestore";
...
const Addition = (props) => {
    ...
    const documentSnapshot =  firebase.firestore().collection('checkbox').doc(uid)

    documentSnapshot.get().then((doc)=>{
       if (doc.exists) {
         const DocData = doc.data();
         const InitialData = DocData["checked"];
    ...
    const [selected, setSelected] = React.useState([init]);
