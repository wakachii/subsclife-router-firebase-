...
import { getFirestore, collection, addDoc,setDoc,onSnapshot } from "firebase/firestore";
...
const Addition = (props) => {
    ...
    const docId = db.collection("checkbox").doc().id;

    const onClickAdd = async() => {
        try {
          const docRef =  await addDoc(collection(db, "checkbox"), {
            checked:selected,
            uid: uid
          });
      ...
      const update = async (uid) => {
        const userCollectionRef = collection(db, 'checkbox');
        const q = query(userCollectionRef, where('name', '==', uid));
