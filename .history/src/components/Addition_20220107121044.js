...
const Addition = (props) => {
    ...
    useEffect (() => {
        getSubscData();}, []);
          ...
          //firebase subscData
    const getSubscData = () => {
    const subscData = db.collection('subscItems')
    subscData.where('uid', '==', currentUser.uid).orderBy('date').startAt(startOfMonth(date)).endAt(endOfMonth(date)).onSnapshot(query => {
      const subscItems = []
      query.forEach(doc => subscItems.push({...doc.data(), docId: doc.id}))
      setsubscItems(subscItems);
    })
  }
